...
<%
    ...
    // 获取待购商品列表
    List<com.barbie.model.Product> pendingProducts = new java.util.ArrayList<>();
    String pendingIdsStr = look != null ? look.getPendingIds() : "";
    if (pendingIdsStr != null && !pendingIdsStr.isEmpty()) {
        com.barbie.dao.ProductDao productDao = new com.barbie.dao.ProductDao();
        for (String id : pendingIdsStr.split(",")) {
            com.barbie.model.Product p = productDao.findById(Integer.parseInt(id.trim()));
...
<div class="app">
    <div class="preview-area">
        ...
        <div class="look-name-input" id="lookNameInput" contenteditable="true"><%= look != null ? look.getName() : "我的搭配" %></div>
        ...
        <div class="preview-footer">
            ...
            <div>
                ...
                <button class="btn-save" onclick="saveLook()">💾 保存修改</button>
            </div>
        </div>
    </div>
        ...
        <div style="margin-top:12px;">
            ...
            <div class="scene-season">
                <select id="sceneSelect">
                    ...
                    <option value="通勤" <%= "通勤".equals(look != null ? look.getScene() : "") ? "selected" : "" %>>通勤</option>
                    <option value="街头" <%= "街头".equals(look != null ? look.getScene() : "") ? "selected" : "" %>>街头</option>
                    <option value="约会" <%= "约会".equals(look != null ? look.getScene() : "") ? "selected" : "" %>>约会</option>
                    <option value="运动" <%= "运动".equals(look != null ? look.getScene() : "") ? "selected" : "" %>>运动</option>
                    <option value="度假" <%= "度假".equals(look != null ? look.getScene() : "") ? "selected" : "" %>>度假</option>
                </select>
                <select id="seasonSelect">
                    <option value="">请选择</option>
                    <option value="春" <%= "春".equals(look != null ? look.getSeason() : "") ? "selected" : "" %>>春</option>
                    <option value="夏" <%= "夏".equals(look != null ? look.getSeason() : "") ? "selected" : "" %>>夏</option>
                    <option value="秋" <%= "秋".equals(look != null ? look.getSeason() : "") ? "selected" : "" %>>秋</option>
                    <option value="冬" <%= "冬".equals(look != null ? look.getSeason() : "") ? "selected" : "" %>>冬</option>
                    <option value="四季" <%= "四季".equals(look != null ? look.getSeason() : "") ? "selected" : "" %>>四季</option>
                </select>
            </div>
        </div>
...
<!-- 隐藏字段 -->
<input type="hidden" id="wardrobeIds" value="<%= look != null ? look.getWardrobeIds() : "" %>">
<input type="hidden" id="pendingIds" value="<%= look != null ? look.getPendingIds() : "" %>">
...
<script>
    ...
    var lookId = <%= look != null ? look.getId() : 0 %>;

    <%
        StringBuilder jsWardrobe2 = new StringBuilder("[");
        if (wardrobeList != null) {
            for (int i = 0; i < wardrobeList.size(); i++) {
                ...
                if (i > 0) jsWardrobe2.append(",");
                jsWardrobe2.append("{id:").append(w.getId())
    ...
    allWardrobe = <%= jsWardrobe2.toString() %>;

    // 初始化已有衣服
    var initialOwned = document.getElementById('wardrobeIds').value;
    if (initialOwned) {
        ownedIds = initialOwned.split(',').filter(function(id) { return id !== ''; });
    }

    // 初始化待购商品
    var initialPending = document.getElementById('pendingIds').value;
    if (initialPending && initialPending.trim() !== '') {
        var ids = initialPending.split(',').filter(function(id) { return id !== ''; });
        <%
            if (pendingProducts != null) {
                for (com.barbie.model.Product p : pendingProducts) {
                    String img = p.getImages();
                    if (img != null && !img.isEmpty() && !img.startsWith("uploads/")) {
                        img = "uploads/" + img;
                    }
        %>
        pendingIds.push('<%= p.getId() %>');
        pendingProducts.push({
            id: '<%= p.getId() %>',
            name: '<%= p.getName().replace("'", "\\'") %>',
            img: '<%= img %>',
            price: <%= p.getPrice() %>,
            category: '<%= p.getCategory() != null ? p.getCategory() : "上装" %>',
            source: 'pending'
        });
        <%
                }
    ...
    function saveLook() {
        ...
        form.method = 'POST';
        form.action = '${pageContext.request.contextPath}/look/edit';
        form.innerHTML =
            '<input type="hidden" name="id" value="' + lookId + '">' +
